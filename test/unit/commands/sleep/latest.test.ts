import {describe, it, expect, vi, beforeEach} from 'vitest';
import {Config} from '@oclif/core';

// Mock modules BEFORE importing the command
vi.mock('../../../../src/lib/auth.js', () => ({
  loadAuth: vi.fn(() => ({access_token: 'test-token', expires_at: Date.now() + 3_600_000})),
  getValidToken: vi.fn(async () => 'test-token'),
  resetAuthCache: vi.fn(),
}));
vi.mock('../../../../src/lib/config.js', () => ({
  loadConfig: vi.fn(() => ({})),
  resetConfigCache: vi.fn(),
}));

const mockApi = {
  listSleeps: vi.fn(),
};
vi.mock('../../../../src/lib/api.js', () => ({
  getApi: vi.fn(() => mockApi),
  WhoopApiError: class WhoopApiError extends Error {
    constructor(message: string, public statusCode: number, public retryAfter?: number) {
      super(message); this.name = 'WhoopApiError';
    }
  },
  resetApi: vi.fn(),
}));

import SleepLatest from '../../../../src/commands/sleep/latest.js';
import {makeSleep, makePendingSleep, paginated} from '../../../helpers/fixtures.js';

describe('sleep latest', () => {
  let stdout: string[];

  beforeEach(() => {
    stdout = [];
    vi.clearAllMocks();
  });

  function makeConfig(): Config {
    return {runHook: vi.fn().mockResolvedValue({successes: []})} as unknown as Config;
  }

  async function runCommand(argv: string[] = []): Promise<string[]> {
    const cmd = new SleepLatest(argv, makeConfig());
    cmd.log = (msg?: string) => { stdout.push(msg ?? ''); };
    cmd.error = ((msg: string) => { throw new Error(msg); }) as any;
    await cmd.init();
    await cmd.run();
    return stdout;
  }

  it('should display no data message when no sleeps exist', async () => {
    mockApi.listSleeps.mockResolvedValue(paginated([]));

    const output = await runCommand();
    const text = output.join('\n');

    expect(text).toContain('No sleep data found.');
    expect(mockApi.listSleeps).toHaveBeenCalledWith({limit: 1});
  });

  it('should output sleep performance in quiet mode', async () => {
    const sleep = makeSleep();
    mockApi.listSleeps.mockResolvedValue(paginated([sleep]));

    const output = await runCommand(['--quiet']);
    const text = output.join('\n');

    // quiet mode outputs Math.round(sleep_performance_percentage) = 94
    expect(text).toBe('94');
  });

  it('should output score_state in quiet mode for pending sleep', async () => {
    const pending = makePendingSleep();
    mockApi.listSleeps.mockResolvedValue(paginated([pending]));

    const output = await runCommand(['--quiet']);
    const text = output.join('\n');

    expect(text).toBe('PENDING_SCORE');
  });

  it('should display scored sleep in CSV format (delegates to SleepGet)', async () => {
    const sleep = makeSleep();
    mockApi.listSleeps.mockResolvedValue(paginated([sleep]));

    const output = await runCommand(['--csv']);
    const text = output.join('\n');

    // CSV uses this.printFormatted which calls this.log on the original command
    expect(text).toContain('Date,Sleep ID,Cycle ID');
    expect(text).toContain('2024-06-14');
    expect(text).toContain('7h 05m');
  });
});
